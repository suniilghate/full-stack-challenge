...
async function fetchProducts() {
    try {
        const response = await fetch(localhost + "/api/data-info.php?fetch=business");
...
function renderPage(pageNumber, data) {
    ...
    productsToShow.forEach(product => {
        const year = product.registration_date.split("-")[0];
        const row = document.createElement("tr");
        row.innerHTML = `<td>${product.id}</td>
        <td><a href="${localhost}/public/index.php?fetch=single-business&id=${product.id}" class="page-link">${product.name}</a></td>
        <td>${product.registered_address}</td>
        <td><a href="${localhost}/public/index.php?fetch=businesses-registered-in-year&id=${year}" class="page-link">${product.registration_date}</a></td>
...
function updateProductListing(data) {
    ...
    if (searchTerm) {
        filteredProducts = filteredProducts.filter(product => 
            ...
            product.registered_address.toLowerCase().includes(searchTerm) ||
            product.registration_date.toLowerCase().includes(searchTerm) ||
    ...
    filteredProducts.sort((a, b) => {
        ...
        } else if (sortBy === "baddress") {
            return a.registered_address.localeCompare(b.registered_address);
        } else if (sortBy === "bregistration-date") {
            return a.registration_date.localeCompare(b.registration_date);
        } else if (sortBy === "bregistration-no") {
